...
/**
 * @mainpage TinyJobs Documentation
 *
 * TinyJobs is a header-only C++ job system providing:
 * * DAG-based scheduling
 * * Core-pinned worker threads
 * * Optional fiber support
 * 
 * For information on getting started, check the `README`.
 */
...
#if defined(DEBUG) || defined(_DEBUG) || defined(__DEBUG) || defined(JOBS_DEBUG) || defined(_TINY_JOBS_DEBUG) || defined(__TINY_JOBS_DEBUG)
...
public:
     ...
     * 
     * @throws @ref `Tiny::Jobs::Exception` An exception is thrown in debug mode if a type mismatch is detected or the future is invalid. 
     * 
...
public:
     ...
     * 
     * @throws @ref `Tiny::Jobs::Exception` If one of the dependency indices does not name a known task
     * 
     ...
     * 
     * @throws @ref `Tiny::Jobs::Exception` In debug when the maximum amount of direct children of a single node is surpassed. 
     */
    void finalize() {
...
public:
     ...
     : m_count(count)
    {
        ...
        for (size_t i = 0; i < m_count; ++i) {
            ...
            {new (m_tasks + i) Task(std::forward<Func>(fn), i, std::forward<Args>(args)...);}
...
protected:
     ...
     * 
     * @throws @ref `Tiny::Jobs::Exception` If this function throws, please report it to the developers as this should not happen. This only throws if the scheduler messes up critically. 
     * 
